//
// V3: 2 independent hashes per thread for instruction-level parallelism.
// The two hashes have no data dependencies between them, so the compiler/
// scheduler can interleave their instructions to hide latency from chained
// 64-bit add+rotate dependencies within each G-function.
//
// Trade-off: 2× register pressure. On Apple GPU SIMT (32-bit native), each
// ulong is a pair of uint registers; total per-thread state is roughly
// 2 × 16 v-registers × 2 uint = 64 uint registers. Acceptable.
//
...
// Each G applied to one hash (suffix A or B distinguishes the two streams).
#define G2(suf, a, b, c, d, x, y) do {                            \
    v##a##suf = v##a##suf + v##b##suf + (x);                      \
    v##d##suf = rotr64(v##d##suf ^ v##a##suf, 32);                \
    v##c##suf = v##c##suf + v##d##suf;                            \
    v##b##suf = rotr64(v##b##suf ^ v##c##suf, 24);                \
    v##a##suf = v##a##suf + v##b##suf + (y);                      \
    v##d##suf = rotr64(v##d##suf ^ v##a##suf, 16);                \
    v##c##suf = v##c##suf + v##d##suf;                            \
    v##b##suf = rotr64(v##b##suf ^ v##c##suf, 63);                \
} while (0)

// One round = 8 G's per hash, interleaved A/B so the scheduler can overlap.
...
kernel void blake2b256_v3(
    device   const uchar* inputs   [[buffer(0)]],
    device         uchar* outputs  [[buffer(1)]],
    constant       uint&  count    [[buffer(2)]],
    uint                  gid      [[thread_position_in_grid]]
) {
    // Each thread processes 2 hashes at indices 2*gid and 2*gid+1.
    uint idxA = gid * 2u;
    uint idxB = idxA + 1u;
    if (idxA >= count) return;

    // Load message A (32 bytes)
    ...
    // Load message B (may be past end if count is odd; mask later)
