//
// V4 mining kernel — explicit 4-load batching to force memory-level parallelism.
//
// V1's structure has 32 sequential (load → add) iterations, which lets
// the GPU schedule maybe 1-2 outstanding loads per thread. By precomputing
// the 4 indexes, issuing 4 loads in a row, then 4 adds, we force the
// compiler to keep 4 loads in flight per thread → 4× MLP per thread.
//
// Apple GPU SIMT can sustain ~16 outstanding loads per thread, so this
// is well within the architectural budget. Register temp footprint:
// 4 × ulong4 = 16 ulongs = 32 registers held during each batch — within
// the per-thread register budget.
//
// Single nonce per thread (avoids V2's register-pressure regression).
//
...
kernel void mine_kernel_v4(
    ...
    #define IDX(i) ((ulong)((uint)((SBYTE(i  ) << 24) |  \
                                  (SBYTE(i+1) << 16) |  \
                                  (SBYTE(i+2) <<  8) |  \
                                   SBYTE(i+3))) % p.n)
    ...
    // Batched 4-up: compute 4 indexes, issue 4 loads, then 4 adds.
    // The 4 ulong4 temporaries (16 ulongs total) live across the load
    // barrier so the compiler must keep 4 loads outstanding per thread.
    #define BATCH(a,b,c,d) do {                          \
        ulong ia = IDX(a);                               \
        ulong ib = IDX(b);                               \
        ulong ic = IDX(c);                               \
        ulong id = IDX(d);                               \
