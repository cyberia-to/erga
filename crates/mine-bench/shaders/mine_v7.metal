//
// V7 DIAGNOSTIC — V1 with R loads stripped (use constant fake rows).
//
// Same Blake2b structure as V1, but skip the 32 random table reads and
// add a constant value to the sum instead. Compares to V1 to measure
// pure compute cost (both Blake2b passes + genIndexes + sum) without
// any memory bandwidth contribution.
//
// Correctness: NOT a real Autolykos miner. Diagnostic only.
//
...
kernel void mine_kernel_v7(
    ...
    // Full Blake2b just like V1 — same compute load.
    ulong m_blk[16];
    ...
    // Skip the 32 R loads. Use a "synthetic" row that varies per
    // iteration (so dead-code-elim can't collapse the sum). The added
    // value is derived from seed_h to keep data dependencies similar.
    ulong sum[4] = {0,0,0,0};
    // Touch R[0] once just so the buffer binding isn't optimized away.
    ulong4 r0 = R[0];
    ulong v0 = seed_h[0] ^ r0.x;
    ulong v1 = seed_h[1] ^ r0.y;
    ulong v2 = seed_h[2] ^ r0.z;
    ulong v3 = seed_h[3] ^ r0.w;
    for (uint i = 0; i < 32; i++) {
        // Synthetic row; no memory dependency on i.
        ulong4 row;
        row.x = v0 + (ulong)i;
        row.y = v1 ^ (ulong)i;
        row.z = v2 + (ulong)i * 0x9E3779B97F4A7C15UL;
        row.w = v3 ^ (ulong)i * 0xBF58476D1CE4E5B9UL;
        add256_v4(sum, row);
    }
